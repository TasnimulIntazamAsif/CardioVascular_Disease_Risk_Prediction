    ...
    try:
        ...
        for feature in lifestyle_features:
            ...
            if feature in df.columns:
                ...
                lifestyle_data.append(df[f'{feature}_encoded'].iloc[0])
            else:
                lifestyle_data.append(0.0)
        
        # Convert to DataFrame with proper column names for scaling
        X_clinical_df = pd.DataFrame([clinical_data], columns=clinical_features)
        X_lifestyle_df = pd.DataFrame([lifestyle_data], columns=lifestyle_features)
        
        # Scale the features separately
        try:
            Xc_scaled = scaler_clinical.transform(X_clinical_df)
            Xl_scaled = scaler_lifestyle.transform(X_lifestyle_df)
        except Exception as e:
            print(f"Scaling error: {e}")
            # If scalers are not fitted, fit them on dummy data
            dummy_clinical = pd.DataFrame(np.zeros((1, len(clinical_features))), columns=clinical_features)
            dummy_lifestyle = pd.DataFrame(np.zeros((1, len(lifestyle_features))), columns=lifestyle_features)
        ...
        print(f"Debug - Clinical features: {clinical_features}")
        print(f"Debug - Clinical data: {clinical_data}")
        print(f"Debug - Lifestyle features: {lifestyle_features}")
        print(f"Debug - Lifestyle data: {lifestyle_data}")
        print(f"Debug - Final fused shape: {X_fused.shape}")
