...
def load_model_and_preprocessors():
    ...
    try:
        ...
        if os.path.exists('models/rf_improved_model.joblib'):
            model = joblib.load('models/rf_improved_model.joblib')
            print("Loaded improved Random Forest model with balanced predictions")
            print(f"Model type: {type(model)}")
            print("Model accuracy: 58.88% (improved for balanced predictions)")
        elif os.path.exists('models/rf_weighted_fusion_model.joblib'):
        ...
        # Load scalers - try improved first, then fallback to original
        if os.path.exists('models/scaler_clinical_improved.joblib'):
            scaler_clinical = joblib.load('models/scaler_clinical_improved.joblib')
            print("Loaded improved clinical features scaler")
        elif os.path.exists('models/scaler_clinical.joblib'):
        ...
        if os.path.exists('models/scaler_lifestyle_improved.joblib'):
            scaler_lifestyle = joblib.load('models/scaler_lifestyle_improved.joblib')
            print("Loaded improved lifestyle features scaler")
        elif os.path.exists('models/scaler_lifestyle.joblib'):
        ...
        # Load fusion parameters - try improved first, then fallback to original
        if os.path.exists('models/fusion_params_improved.joblib'):
            fusion_params = joblib.load('models/fusion_params_improved.joblib')
            print(f"Loaded improved fusion parameters - Clinical weight: {fusion_params['weight_clinical']:.4f}, Lifestyle weight: {fusion_params['weight_lifestyle']:.4f}")
        elif os.path.exists('models/fusion_params.joblib'):
